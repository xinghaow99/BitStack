        ...
        for name, module in named_linears.items():
            ...
            if init_only:
                if compression_config is not None:
                    ...
                    load_wbits = wbits
                qlinear = BitStackLinear.from_linear(module, w_bit=load_wbits, k=k, bias=False, no_avd=signed, fused_level=fused_level, name=layer_name, init_only=init_only)
                set_op_by_name(layer, name, qlinear)
            else:
                module.cuda()
                qlinear = BitStackLinear.from_linear(module, w_bit=wbits, k=k, bias=False, no_avd=signed, fused_level=fused_level, name=layer_name, init_only=init_only)
                module.cpu()
                qlinear.cpu()
                set_op_by_name(layer, name, qlinear)
